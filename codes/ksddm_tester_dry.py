from codes.common import load_and_prepare_dataset, define_batches
from codes.config import insects_datasets
from codes.KSDDM import KSDDM
...
def plot_statistics(
    ...
    ax.plot(
        "Batch", "minimum_values", data=plot_data, label="minimum_values", marker="."
    )
    ax.plot(
        "Batch", "maximum_values", data=plot_data, label="maximum_values", marker="."
    )
...
def plot_values(
    plot_data, batches_list, dataset_name, batch_size, change_points, value_to_plot=""
):
    sns.set_style("whitegrid")
    fig, ax = plt.subplots(figsize=(20, 6))
...
# from codes.ddm import fetch_ksddm_drifts
#
#
